...
## FIGURE 1
subfolder = fig_folder+"fig1_Li100/"
with open(subfolder+"ecdec_fitparams.txt","r") as f:
...
dos = pd.read_csv(subfolder+"dos_interp.txt", sep='\t', skiprows=1, names=["E", "DOS"])

# data for figure b, current / rate constant
exp_data = pd.read_csv(subfolder+"ecdec.txt", names=["V", "I"])
fit_data = pd.read_csv(subfolder+"ecdec_fitdata.txt", sep='\t', skiprows=1, names=["V", "MHC_k", "MHC_DOS_k"])
...
a1.set_xlabel("Voltage [V]", fontsize=lb_fs)
a1.tick_params(labelsize=tl_fs)
a1.set_xlim([-3, 3])
plt.scatter(exp_data.V, exp_data.I, c=[[0,0,0,0]], edgecolors='k', linewidths=1, zorder=10)
plt.legend(["MHC", "MHC+DOS", "expt."], fontsize=tl_fs-1)
plt.tight_layout()
# and the inset
