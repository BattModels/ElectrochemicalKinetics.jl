...
function rate_constant(V_app::Real, amhc::AsymptoticMarcusHushChidsey, ::Val{ox}; kT::Real = 0.026) where ox
    η = (2 * ox - 1) .* V_app ./ kT
    λ_nondim = amhc.λ / kT
    a = 1 .+ sqrt.(λ_nondim)
...
function rate_constant(V_app::Real, amhc::AsymptoticMarcusHushChidsey; kT::Real = 0.026)
    η = V_app / kT
    λ_nondim = amhc.λ / kT
    a = 1 .+ sqrt.(λ_nondim)
    arg = (λ_nondim .- sqrt.(a .+ η.^2)) ./ (2 .* sqrt.(λ_nondim))
